...
import { generateDayTabs, getWeekRange, navigateWeek } from '../utils/weekViewUtils';
...
export function CalendarController({
  ...
  return (
    <div className="flex flex-col items-center space-y-4 w-full max-w-md overflow-hidden">
      ...
      {/* Day Filter Tabs */}
      <div className="flex space-x-2 max-w-full overflow-x-auto justify-between  scrollbar-none">
        {dayTabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={` py-2 text-sm rounded font-medium transition lowercase
                flex flex-col items-center justify-center shrink-0 w-14 gap-0.5 cursor-pointer border-whitesmoke/15 border
                ${isActive 
